...
class TinyFlowRefiner(nn.Module):
    """
    Super-light full-resolution flow refiner.
    Inputs: concat([guidance_8ch, flow_2ch]) -> delta_flow (2ch)
    """
    def __init__(self, in_ch=10, mid_ch=16, num_blocks=2, use_se=True):
        super().__init__()
        blocks = []
        ch = mid_ch
        blocks += [DepthwiseSeparableConv(in_ch, ch, k=3)]
        # a couple of DSConv residual blocks with dilation to kill blockiness
        for d in [1, 2][:num_blocks]:
            blocks += [
...
class LiteFlowHead(nn.Module):
    ...
    def __init__(
        ...
        # --- output-stage tiny refiner (optional) ---
        self.enable_output_refine = True  # set False if you prefer the existing refine_up
        self.guidance_ch = 8              # tiny guidance
        # reduce f1/f2 (projected) to tiny guidance; created lazily because proj dims are known
        self.guid_red = None
        self.out_refiner = TinyFlowRefiner(in_ch=2 + self.guidance_ch, mid_ch=16, num_blocks=2)

    def _make_guid_red(self, in_ch: int, device: torch.device):
        # single 1x1 to squeeze concat([f1,f2]) -> guidance_ch
        if self.guid_red is None:
            self.guid_red = nn.Conv2d(in_ch, self.guidance_ch, 1, bias=False).to(device)
        return self.guid_red
    ...
    def forward(
        ...
        H_img, W_img = self.out_size
        flow_img = F.interpolate(flow_coarse, size=(H_img, W_img), mode='bilinear', align_corners=True)
        flow_img[:,0] *= scale_w
        flow_img[:,1] *= scale_h

        # # ---- FINAL TINY OUTPUT REFINER (full-res, edge-aware) ----
        # if self.enable_output_refine:
        #     # Build tiny guidance from (f1,f2): reduce then upsample
        #     guid_in = torch.cat([f1, f2], dim=1)                    # (B, 2*proj_channels, H, W)
        #     guid_red = self._make_guid_red(guid_in.shape[1], guid_in.device)(guid_in)
        #     guid_up  = F.interpolate(guid_red, size=(out_h, out_w), mode='bilinear', align_corners=False)

        #     # Optionally, add simple spatial gradients to sharpen edges (no params)
        #     sobel_x = torch.tensor([[1,0,-1],[2,0,-2],[1,0,-1]], dtype=guid_up.dtype, device=guid_up.device).view(1,1,3,3)
        #     sobel_y = torch.tensor([[1,2,1],[0,0,0],[-1,-2,-1]], dtype=guid_up.dtype, device=guid_up.device).view(1,1,3,3)
        #     # depthwise sobel on each guidance channel
        #     gx = F.conv2d(guid_up, sobel_x.repeat(guid_up.shape[1],1,1,1), padding=1, groups=guid_up.shape[1])
        #     gy = F.conv2d(guid_up, sobel_y.repeat(guid_up.shape[1],1,1,1), padding=1, groups=guid_up.shape[1])
        #     grad_mag = torch.sqrt(gx*gx + gy*gy + 1e-6).mean(1, keepdim=True)  # 1 ch

        #     tiny_guid = torch.cat([guid_up, grad_mag], dim=1)[:, :self.guidance_ch, ...]  # keep it tiny
        #     ref_in = torch.cat([tiny_guid, flow_up], dim=1)  # (B, 8+2, H, W)
        #     flow_up = flow_up + self.out_refiner(ref_in)
