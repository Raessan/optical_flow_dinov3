...
# ----------------------------
# Param-free TV smoothing (optional)
# ----------------------------

@torch.no_grad()
def tv_step(flow: torch.Tensor, guide: torch.Tensor, lam: float = 0.02, eps: float = 1e-6, step: float = 0.5):
    """
    One step of edge-aware total-variation smoothing.
    flow: (B,2,H,W), guide: (B,1,H,W) in [0,1]
    """
    gx = F.pad(guide[:, :, :, 1:] - guide[:, :, :, :-1], (0, 1, 0, 0))
    gy = F.pad(guide[:, :, 1:, :] - guide[:, :, :-1, :], (0, 0, 0, 1))
    w = 1.0 / torch.sqrt(gx.pow(2) + gy.pow(2) + eps)

    fx = F.pad(flow[:, :, :, 1:] - flow[:, :, :, :-1], (0, 1, 0, 0))
    fy = F.pad(flow[:, :, 1:, :] - flow[:, :, :-1, :], (0, 0, 0, 1))

    divx = fx * w
    divy = fy * w
    div  = (F.pad(divx[:, :, :, :-1] - divx[:, :, :, 1:], (1, 0, 0, 0)) +
            F.pad(divy[:, :, :-1, :] - divy[:, :, 1:, :], (0, 0, 1, 0)))
    return flow + step * lam * div
...
class LiteFlowHead(nn.Module):
    ...
    def __init__(
        ...
        tv_steps: int = 0,             # param-free smoothing steps at full-res
    ):
        super().__init__()
    ...
    def forward(self, feat1: torch.Tensor, feat2: torch.Tensor) -> torch.Tensor:
        ...
        # Optional param-free TV smoothing (edge-aware) using f1 as guidance
        if self.tv_steps > 0:
            with torch.no_grad():
                guide = F.interpolate(f1.mean(1, keepdim=True), size=(out_h, out_w), mode='bilinear', align_corners=False)
                guide = (guide - guide.min()) / (guide.max() - guide.min() + 1e-6)
                for _ in range(self.tv_steps):
                    flow = tv_step(flow, guide, lam=0.02, step=0.5)
